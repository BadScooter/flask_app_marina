...
def xgboost_func(features, numlags, option):
    marina = pd.read_csv("./website/MarinaDataset.csv", parse_dates=["datetime"])
    ...
    # calculate RMSE
    rmse = sqrt(mean_squared_error(test.arrivals, predictions))

    return plot_png1, plot_png2, rmse